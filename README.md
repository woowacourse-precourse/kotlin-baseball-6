...
- 입력
- [x] 서로 다른 세 자리의 수 
- [x] 게임이 끝나면 재시작/종료(1,2)로 구분
- [x] 잘못된 값을 입력하면 `IllegalArgumentException`을 발생 후 애플리케이션 종료
  - 게임 입력값
    - [x] 3자리 숫자가 아닌 경우(ex. 공백을 제외한 문자열의 길이가 3이 아닌 경우)
    - [x] 공백을 제외하고 숫자 3자리인 경우 (ex "123 ", " 123" 등.)
    - [x] 숫자 **0** 을 입력한 경우 **(게임에 0이 사용되지 않음)**
    - [x] 숫자가 아닌 입력을 넣은 경우
    - [x] 입력된 세 숫자 안에서 동일한 값이 존재하는 경우
  - 게임 재시작 / 종료 조건
    - [x] **1** 또는 **2** 가 아닌 경우
...
- [x] 볼과 스트라이크가 둘다 있다면 볼을 먼저 출력
- [x] 같은 수가 전혀 없으면 "낫싱" 출력
- [x] 3개의 숫자를 모두 맞힐 경우 게임 종료 문구 출력


- 기타 기능
- [x] 랜덤 숫자의 각 자리 수는 '1'에서 '9' 사이의 숫자
- [x] 랜덤 숫자의 각 숫자는 중복되지 않음

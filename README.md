...
---
## 구현해야 하는 목록
1. 숫자 야구 게임을 시작한다는 문구, 숫자를 입력하라는 문구, 입력할 readLine() 메소드를 사용해서 숫자를 입력받아야 한다.
2. 컴퓨터가 생각해야 할 서로 다른 3개의 숫자 생성해야 한다. 범위는 1 ~ 9까지의 범위로.
3. 게임을 한번 실행한 후, 3개의 숫자와 위치를 맞췄다면 게임을 종료 or 게임을 새로 진행하는 함수를 짜야하고, 못 맞췄다면 게임을 계속 진행해야 되기 때문에 do-while문으로 못맞추면 계속 반복되게 해야 한다.
4. 컴퓨터가 생각한 숫자들을 answer로, 내가 그 숫자들을 추측하는 것을 guess로 가정하고, 스트라이크, 볼, 낫싱의 조건을 맞춰야 한다.
5. 게임이 종료되었다면, 게임을 1번을 눌러 다시 시작할 것인지, 2번을 눌러 종료시킬 것인지 물어보게해야 한다. 
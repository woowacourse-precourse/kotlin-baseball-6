...
- Random 메소드 사용해서 무작위로 숫자 3개를 생성한 리스트에 저장하기
- 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생하기
...
- 숫자가 3개 몸두 일치한 경우, 반복 끝내고 "새로 시작: 1, 종료: 2" 출력하기
- 숫자가 존재하고 인덱스 위치가 다르면 "볼", 같으면 "스트라이크" 출력하기
- Random 메소드에서 **pickNumberInRange()** 무작위로 숫자 3개를 생성한 리스트에 저장하기
- 사용자가 잘못된 값을 입력한 경우, **IllegalArgumentException**을 발생시킨 후 애플리케이션 종료하기
- 사용자 입력 부분은 **readLine()** 을 활용하기
- 사용자가 중복된 숫자를 입력할 경우 검사하기
- 사용자 입력에 0이 들어올 경우 검사하기

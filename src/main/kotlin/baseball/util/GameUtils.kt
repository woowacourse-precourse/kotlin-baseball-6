...
object GameUtils {
    ...
    /**
     * getRandomList()
     * 게임에서 사용할 1에서 9까지 서로 다른 임의의 3개 숫자를 생성한다.
     *
     * @return 1에서 9까지 서로 다른 3개의 숫자로 이루어진 리스트
     */
    ...
    /**
     * inputNum()
     * 사용자가 입력한 연속된 3자리의 문자열을 받아 한 글자씩 분리하여 리스트 형태로 반환한다.
     *
    ...
    /**
     * isValidInput()
     * 유효한 입력(길이:3, 입력형태:Int, 중복:X)인지 확인 후 유효할 경우 true, 그렇지 않을 경우 false를 반환한다.
     *
     * @param inputList 확인할 입력 문자열
     * @return 입력이 서로다른 3자리이고, 숫자로만 구성된 경우 true를 반환
     */
    private fun isValidInput(inputList: String): Boolean {
        return inputList.toSet().size == 3 && inputList.all { it.isDigit() }
    }


    /**
     * compareResult()
     * 사용자가 입력한 숫자와 컴퓨터가 선택한 숫자를 비교하여 결과를 계산한다.
     *
    ...
    /**
     * restartGame()
     * 사용자로부터 게임 재시작 또는 종료를 입력받는 함수이다.
     *

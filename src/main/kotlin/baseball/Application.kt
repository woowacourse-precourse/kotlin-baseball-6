...
fun main() {
    ...
    while (start) {
        // 숫자 무작위로 1부터 9까지 선택 (컴퓨터 상대)
        val comNums: List<Int> = listOf(7, 1, 3)
        ...
        while (!Input) { // Input이 참이면 계속 반복
            ...
            // toIntOrNull()을 이용해서 입력한 문자열이 정수 변환이 되는지(숫자로만 이루어져 있는지) 확인
            if (userInput.length == 3 && userInput.toIntOrNull() != null) {
                Input = checkNums(comNums, userInput.map { it.toString().toInt() })
            } else {
